    ...
    class Human final: public Framework::Integrations::Scripting::Entity {
      public:
        ...
        void AddWeapon(int weaponId, int ammo) {
            FW_SEND_SERVER_COMPONENT_GAME_RPC(Sandbox::Shared::RPC::AddWeapon, _ent, weaponId, ammo);
        }

        void SetHealth(float health) {
            auto h = _ent.get_mut<Sandbox::Shared::Modules::HumanSync::UpdateData>();
            h->_healthPercent = health;
            FW_SEND_SERVER_COMPONENT_GAME_RPC(Sandbox::Shared::RPC::HumanSetProps, _ent, health);
        }

        float GetHealth() const {
            auto h = _ent.get<Sandbox::Shared::Modules::HumanSync::UpdateData>();
